...
class QuantizedLinear(SimpleQuantizedModel):
    ...
    def forward(self, x: 'npt.NDArray[np.int32]', bit_shift: int) -> 'tuple[npt.NDArray[np.int32], int]':
        ...
        current_shift = 0
        while (actual_max / (1 << current_shift)) > self.max_input:
            current_shift += 1
        total_shift = bit_shift + current_shift
        y = (self.bias / (1 << total_shift)) + np.dot(self.weight, x / (1 << current_shift))
...
def quantize_linear(linear: nn.Linear, input_factors: 'npt.NDArray[np.float64]') -> QuantizedLinear:
    ...
    while True:
        ...
        weight = np.round(weight)#weight = np.round(weight).astype(np.int64)
        pprint((output_factors, weight))
        bias = initial_bias * output_factors
        bias = np.round(bias)#bias = np.round(bias).astype(np.int64)
...
def test():
    ...
    pprint(letter_embedding * letter_factors)

    relu = QuantizedReLU()
    group_inter_linear = quantize_linear(model.group_inter_linear, np.concat([letter_factors] * (model.group_inter_linear.in_features // len(letter_embedding[0]))))
